...
const loginToSchool = async (username, password, chatId) => {
  ...
  try {
    ...
    if (page.url().includes("daotao.qnu.edu.vn/Home")) {
...
const checkNewGrades = async (chatId) => {
  ...
  try {
    await page.goto("https://daotao.qnu.edu.vn/Home/Marks", { waitUntil: "networkidle2" });
    ...
    console.error(err);
    bot.sendMessage(chatId, "Đã xảy ra lỗi khi kiểm tra điểm! Vui lòng thử lại.");
  } finally {
    await page.close();
  }
};
...
const handleLogin = async (chatId, username, password) => {
  ...
  if (loginResult) {
    try {
      ...
      await user.save();
      bot.sendMessage(chatId, `Đăng nhập thành công với mã sinh viên ${username}!`);
    } catch (err) {
      console.error("Lỗi khi lưu thông tin người dùng:", err);
      bot.sendMessage(chatId, "Đã xảy ra lỗi khi đăng nhập. Vui lòng thử lại!");
    }
  } else {
    bot.sendMessage(chatId, "Mã sinh viên hoặc mật khẩu không đúng! Vui lòng thử lại bằng lệnh /login.");
  }
...
const toggleGradeCheck = async (chatId) => {
  ...
  if (!user) {
    return bot.sendMessage(chatId, "Bạn chưa đăng nhập! Vui lòng sử dụng lệnh /login để đăng nhập trước.");
  }

  if (!sessions[chatId]?.browser) {
    return bot.sendMessage(chatId, "Browser đã bị đóng hoặc không hoạt động! Vui lòng thử đăng nhập lại để khởi tạo lại browser.");
  }
  ...
  if (checkGradeIntervals[chatId]) {
    ...
    checkGradeIntervals[chatId] = setInterval(() => checkNewGrades(chatId), 600000);
    delete gradeNotifications[chatId]; 
    bot.sendMessage(chatId, "Bật tiến trình kiểm tra điểm tự động 10 phút/1 lần thành công!");
  }
};
...
const handleLogout = async (chatId) => {
  ...
  if (sessions[chatId]) {
    try {
      ...
      const page = await browser.newPage();
      await page.goto("https://daotao.qnu.edu.vn/Login/Logout", { waitUntil: "networkidle2" });
      await browser.close();
      delete sessions[chatId];
    } catch (err) {
      console.error(err);
      return bot.sendMessage(chatId, "Đã xảy ra lỗi khi đăng xuất. Vui lòng thử lại!");
    }
  }
...
bot.onText(/\/login/, async (msg) => {
  ...
  if (await User.findOne({ chatId })) {
    return bot.sendMessage(chatId, "Bạn đã đăng nhập trước đó! Sử dụng lệnh /logout để đăng xuất.");
  }
...
bot.onText(/\/checkaccount/, async (msg) => {
  ...
  bot.sendMessage(msg.chat.id, user ? `Mã sinh viên đang đăng nhập hiện tại: ${user.username}` : "Bạn chưa đăng nhập!");
});

bot.onText(/\/logout/, (msg) => handleLogout(msg.chat.id));

bot.on("message", (msg) => {
  if (msg.text.startsWith("/") && !["/start", "/help", "/login", "/logout", "/checkaccount", "/checknewgrade"].includes(msg.text)) {
    bot.sendMessage(msg.chat.id, `Lệnh \"${msg.text}\" không tồn tại. Sử dụng /help để xem các lệnh hiện có.`);
  }
});
